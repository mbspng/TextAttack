...
def provide_word2vec(embedding_name='cc.en.300.vec'):
    return Word2VecKeyedVectors.load(provide_word2vec_file(embedding_name))


def provide_word2vec_file(embedding_name: str) -> str:
    """Provides the path to a local embedding file for the word2vec fasttext embedding `embedding_name`. If file is not
    present, the embedding is downloaded from the server first.

    Args:
        embedding_name: name of embedding (e.g. 'cc.en.300.vec') to download from server (see https://fasttext.cc).

    Returns: path file embedding file
    """
    ...
    def download_word2vec():
        ...
        def pickle():
            print(f'Saving final embedding as {embedding_path_m}... (This may take a while.)', file=sys.stderr)
            embedding = Word2VecKeyedVectors.load_word2vec_format(embedding_path_vec)
        ...
        Path(os.path.join(tmp_path)).mkdir(parents=True, exist_ok=True)
        download()
        extract()
        pickle()
        clean_up()
    ...
    embedding_path_gz = os.path.join(tmp_path, embedding_name+'.gz')
